...
import type { SourceEngineView, SourceRenderContext } from "../Main.js";
// SourceEngineViewType is also from "../Main.js", but a value import would
// resurrect a Main↔MaterialCache↔MaterialBase cycle that breaks ESM init
// order under Vite's native module loader (TDZ on MaterialShaderTemplateBase).
// The single value-use below uses the literal corresponding to ShadowMap.
const SOURCE_ENGINE_VIEW_TYPE_SHADOW_MAP = 2;
...
export abstract class BaseMaterial {
    ...
    public isMaterialVisible(renderContext: SourceRenderContext): boolean {
        ...
        if (renderContext.currentView.viewType === SOURCE_ENGINE_VIEW_TYPE_SHADOW_MAP) {
