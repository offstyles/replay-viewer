...
export async function parseVMT(filesystem: SourceFileSystem, path: string, depth: number = 0): Promise<VMT> {
    async function parsePath(path: string): Promise<VMT> {
        ...
        // time. Retry obsolete.vmt here so one miss can't reject the batch.
        let bufferOrNull = await filesystem.fetchFileData(path);
        if (bufferOrNull === null && path !== `materials/editor/obsolete.vmt`) {
            path = `materials/editor/obsolete.vmt`;
            bufferOrNull = await filesystem.fetchFileData(path);
        }
        const buffer = assertExists(bufferOrNull);
