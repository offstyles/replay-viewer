...
export function getDebugOverlayCanvas2D(): CanvasRenderingContext2D {
    if (_debugOverlayCanvas === null) {
        ...
        // noclip's app shell exposes `window.main.toplevel` as the container.
        // We're embedded — fall back to document.body so the overlay layer is
        // available for callers (e.g. LuminanceHistogram.debugDraw) without
        // crashing every frame.
        const host = (window as { main?: { toplevel?: HTMLElement } }).main?.toplevel ?? document.body;
...
function shouldCull(p: ReadonlyVec4, clipSpaceNearZ?: number): boolean {
    if (clipSpaceNearZ === undefined) {
        // window.main only exists in noclip's app shell. Fall back to the
        // OpenGL convention (-1) when embedded.
        clipSpaceNearZ = (window as { main?: { viewer?: { gfxDevice?: { queryVendorInfo: () => { clipSpaceNearZ: number } } } } }).main?.viewer?.gfxDevice?.queryVendorInfo().clipSpaceNearZ ?? -1;
    }
