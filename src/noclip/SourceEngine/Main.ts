...
import { GfxRenderInst, GfxRendererLayer, GfxRenderInstList, GfxRenderInstManager, makeSortKey, setSortKeyDepth } from "../gfx/render/GfxRenderInstManager.js";
...
function hasDoubledSegment(path: string): boolean {
    const parts = path.split('/').filter((p) => p.length > 0);
    for (let len = 1; len <= parts.length / 2; len++) {
        for (let start = 0; start + 2 * len <= parts.length; start++) {
            let dup = true;
            for (let k = 0; k < len; k++) {
                if (parts[start + k] !== parts[start + len + k]) {
                    dup = false;
                    break;
                }
            }
...
export class SourceFileSystem {
    ...
    public searchPath(searchDirs: string[], path: string, ext: string): string | null {
        // Two-pass cdir resolution: first authoritative mounts only, so an
        // opportunistic loose mount can't claim a doubled cdir candidate and
        // mask the real material in a later cdir. Second pass falls back to
        // all mounts so CSPakMount-only assets still resolve — without it the
        // MDL loader hardcodes obsolete.vmt
        // for every mesh whose material isn't bundled in the BSP pakfile.
        // Doubled-cdir candidates are deprioritised in the second pass so
        // the real combo wins when it exists.
        const candidates: string[] = [];
        for (let i = 0; i < searchDirs.length; i++) {
            let searchDir = searchDirs[i];
            searchDir = searchDir.replace(/\\/g, '/').replace(/\/\//g, '/');
            if (searchDir.endsWith('/'))
                searchDir = searchDir.slice(0, -1);
            candidates.push(this.resolvePath(`${searchDir}/${path}`, ext));
        }

        for (const c of candidates)
            if (this.hasEntryAuthoritative(c))
                return c;

        // Sort by ascending duplication, then keep input order. First
        // candidate without a doubled segment wins.
        const sorted = candidates
            .map((c, i) => ({ c, i, dup: hasDoubledSegment(c) }))
            .sort((a, b) => (a.dup === b.dup ? a.i - b.i : a.dup ? 1 : -1));
    ...
    public hasEntry(resolvedPath: string): boolean {
        // Authoritative mounts before loose: opportunistic proxies like
        // CSPakMount claim every path and would intercept BSP-pakfile assets.
        for (let i = 0; i < this.vpk.length; i++) {
            const entry = this.vpk[i].findEntry(resolvedPath);
            if (entry !== null)
                return true;
    ...
    public async fetchFileData(resolvedPath: string): Promise<ArrayBufferSlice | null> {
        // See note on hasEntry — same priority order. Authoritative mounts
        // first, opportunistic proxies last.
        for (let i = 0; i < this.vpk.length; i++) {
            const entry = this.vpk[i].findEntry(resolvedPath);
            if (entry !== null)
        ...
        for (let i = 0; i < this.loose.length; i++) {
            ...
            if (custom.hasEntry(resolvedPath)) {
                const data = await custom.fetchEntryData(this.dataFetcher, resolvedPath);
                if (data !== null)
                    return data;
            }
        }
...
export class BSPSurfaceRenderer {
    ...
    public bindMaterial(bspRenderer: BSPRenderer, materialInstance: BaseMaterial, startLightmapPageIndex: number, renderContext: SourceRenderContext): void {
        ...
        for (let i = 0; i < this.surface.faceList.length; i++) {
            ...
            if (lightmapUpdater !== null) {
                ...
                lightmapUpdater.buildLightmap(renderContext, startLightmapPageIndex);
            }
        }
    }
...
export class BSPModelRenderer {
    ...
    public readonly materialsLoaded: Promise<unknown>;
    ...
    constructor(renderContext: SourceRenderContext, public model: Model, private bspRenderer: BSPRenderer, startLightmapPageIndex: number) {
        ...
        this.materialsLoaded = this.bindMaterials(this.bspRenderer, renderContext, startLightmapPageIndex);
    }
    ...
    private async bindMaterials(bspRenderer: BSPRenderer, renderContext: SourceRenderContext, startLightmapPageIndex: number) {
        // allSettled so a single failing material doesn't strand the batch.
        await Promise.allSettled(this.surfaces.map(async (surfaceRenderer) => {
            ...
            await materialInstance.init(renderContext);
            surfaceRenderer.bindMaterial(bspRenderer, materialInstance, startLightmapPageIndex, renderContext);
        }));
    }
...
export class BSPRenderer {
    ...
    public materialsLoaded(): Promise<unknown> {
        return Promise.allSettled(this.models.map((m) => m.materialsLoaded));
    }
...
export class SourceRenderer implements SceneGfx {
    ...
    public render(device: GfxDevice, viewerInput: ViewerRenderInput) {
        ...
        // Force a clear each frame. With the default 'load' clearColor, the
        // post-pass that fills mainColorGammaTargetID was leaving the canvas
        // black (likely the render-graph optimizer eliding passes whose output
        // wasn't observably required). An explicit clear keeps the pipeline
        // honest and gives a known fallback if any single pass goes silent.
        mainColorDesc.clearColor = { r: 0, g: 0, b: 0, a: 1 };
        mainColorDesc.clearDepth = 0;
        ...
        // See note on mainColorDesc above — same reason.
        mainColorGammaDesc.clearColor = { r: 0, g: 0, b: 0, a: 1 };
