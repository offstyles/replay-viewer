...
export class StaticPropRenderer {
    ...
    constructor(renderContext: SourceRenderContext, private bspRenderer: BSPRenderer, private staticProp: StaticProp) {
        // Swallow per-prop failures — third-party maps reference props that
        // aren't in the stock VPK; without this each one is an unhandled rejection.
        this.createInstance(renderContext, bspRenderer).catch(() => { /* missing prop */ });
    }
    ...
    public prepareToRender(renderContext: SourceRenderContext, renderInstManager: GfxRenderInstManager): void {
        ...
        if (!renderContext.currentView.frustum.contains(this.bbox))
            return;
